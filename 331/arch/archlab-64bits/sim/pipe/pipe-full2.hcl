...
# We start by adding IADDQ as a valid instruction, then we add it into the need_regids 
# array since one of the operands is a register, also we add it into need_valC array since
# another operand is an immediate(constant). Also we use the decoded register for rB in
# srcB, since the second operand is a register. And also the same register is used as the
# destination register of the alu so we make sure d_dstE is D_rB for iiaddq. And we pass the
# constant val from execute stage to input A of ALU and valB to second input of alu to peform
# an addition with the constant and the register. Afterwards the result is saved back to the
# same register. IADDQ is a mathematical operation so it must set condition flags so we add 
# it into set_cc condition
...
word M_valA = [
	M_icode == IMRMOVQ && E_icode == IRMMOVQ && M_dstM==E_srcA : m_valM;
	1 : E_valA;
];
...
bool D_bubble =
	...
	!(E_icode in { IMRMOVQ, IPOPQ } && E_dstM in { d_srcA, d_srcB }) &&
	 !(E_icode == IMRMOVQ && D_icode == IRMMOVQ && E_dstM == d_srcA) &&
	  IRET in { D_icode, E_icode, M_icode };
