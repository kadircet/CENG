        ...
        if x['AID'] == 0:
            ...
            for dock in docked:
                if x['SID'] in docked[dock]:
                    ...
                    print x
                    break
            if lastActivity[x['SID']][0] == 0 and abs(x['time stamp']-aTime[x['SID']]) > timeMargin:
                print "Ship did not arrive in time"
                print x, lastActivity[x['SID']], aTime[x['SID']]
                break
            if lastActivity[x['SID']][0] != 0 and abs(lastActivity[x['SID']][1]+tTime[x['SID']]-x['time stamp']) > timeMargin:
                print "Ship did not arrive in time"
    ...
    for x in events:
        if x['AID'] == 0:
            ...
            if x['DID'] not in entrReq or x['SID'] not in entrReq[x['DID']]:
                ...
                print x
                break
            elif x['DID'] in entrReq and entrReq[x['DID']][0] != x['SID']:
                print "Other ship requested first"
                print x, entrReq[x['DID']]
                break
            entrReq[x['DID']] = entrReq[x['DID']][1:]
            docked[x['DID']].append(x['SID'])
        elif x['AID'] == 3:
            if x['DID'] not in unloadReq:
                unloadReq[x['DID']] = []
            ...
            if x['DID'] in lastLoad and lastLoad[x['DID']]+3000>=x['time stamp']:
                ...
                print x
                break
            if x['SID'] not in shipCargo or x['CID'] not in shipCargo[x['SID']]:
                print "Cargo not in ship"
                print x
                break
            unload[x['DID']].append((x['SID'],x['CID']))
            shipCargo[x['SID']].remove(x['CID'])
            lastUnload[x['DID']] = x['time stamp']
        elif x['AID'] == 5:
            if x['DID'] not in lastLoad:
                lastLoad[x['DID']] = 0
            ...
            if x['DID'] in lastUnload and lastUnload[x['DID']]+2000>=x['time stamp']:
                ...
                break
            for y in shipCargo:
                if x['CID'] in shipCargo[y]:
                    print "Cargo in another ship!"
                    print x
                    break
            shipCargo[x['SID']].append(x['CID'])
            load[x['DID']].append((x['SID'],x['CID']))
            lastLoad[x['DID']] = x['time stamp']
        elif x['AID'] == 7:
            if x['DID'] not in leave:
