...
extends ic {
    ...
    public void propagate(CircuitState state) {
        ...
        Value pe = state.getValue(this.getWestPinLoc(4));
        Value _cp1 = state.getValue(this.getSouthPinLoc(0));
        Value _cp2 = state.getValue(this.getSouthPinLoc(1));
        Value ds = state.getValue(this.getSouthPinLoc(2));
        boolean problem = false;
        if (!(p0.isFullyDefined() && p1.isFullyDefined() && p2.isFullyDefined() && p3.isFullyDefined() && pe.isFullyDefined() && _cp1.isFullyDefined() && _cp2.isFullyDefined() && ds.isFullyDefined())) {
        ...
        if (pe.toIntValue() == 0) {
            if (_cp1.toIntValue() == 0 && is._CP1.toIntValue() == 1) {
                Value[] newValues = new Value[]{ds, is.value.get(0), is.value.get(1), is.value.get(2)};
    ...
    extends ic.ICFactory {
        ...
        private static Factory getFactory() {
            ...
            ICDraw.ICPin[] westPins = new ICDraw.ICPin[]{new ICDraw.ICPin(ICDraw.ICPinType.PIN, "P3"), new ICDraw.ICPin(ICDraw.ICPinType.PIN, "P2"), new ICDraw.ICPin(ICDraw.ICPinType.PIN, "P1"), new ICDraw.ICPin(ICDraw.ICPinType.PIN, "P0"), new ICDraw.ICPin(ICDraw.ICPinType.PIN, "PE")};
            ICDraw.ICPin[] eastPins = new ICDraw.ICPin[]{new ICDraw.ICPin(ICDraw.ICPinType.PIN, "Q3"), new ICDraw.ICPin(ICDraw.ICPinType.PIN, "Q2"), new ICDraw.ICPin(ICDraw.ICPinType.PIN, "Q1"), new ICDraw.ICPin(ICDraw.ICPinType.PIN, "Q0")};
            ICDraw.ICPin[] northPins = new ICDraw.ICPin[]{};
            ICDraw.ICPin[] southPins = new ICDraw.ICPin[]{new ICDraw.ICPin(ICDraw.ICPinType.INVERSEPIN, "/CP1"), new ICDraw.ICPin(ICDraw.ICPinType.INVERSEPIN, "/CP2"), new ICDraw.ICPin(ICDraw.ICPinType.PIN, "DS")};
            descriptor = new ICDraw.ICDescriptor(westPins, eastPins, northPins, southPins, "7495");
