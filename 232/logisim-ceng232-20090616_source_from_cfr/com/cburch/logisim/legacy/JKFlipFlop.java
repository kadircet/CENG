...
extends AbstractComponentFactory {
    ...
    public String getName() {
        return "Logisim 1.0 J-K Flip-Flop";
    }

    @Override
    public String getDisplayName() {
        return Strings.get("jkFlipFlopComponent");
    }
    ...
    implements Pokable {
        public Comp(Location loc, AttributeSet attrs) {
            ...
            this.setEnd(2, pt.translate(-30, 10), w, 1);
            this.setEnd(3, pt, w, 2);
            this.setEnd(4, pt.translate(0, 20), w, 2);
        }

        @Override
        public ComponentFactory getFactory() {
            return JKFlipFlop.instance;
        }
        ...
        public void propagate(CircuitState state) {
            ...
            myState.lastClock = clock = state.getValue(this.getEndLocation(2));
            if (lastClock == Value.FALSE && clock == Value.TRUE) {
                Value j = state.getValue(this.getEndLocation(0));
                Value k = state.getValue(this.getEndLocation(1));
                Value curVal = myState.curValue;
                Value newVal = j == Value.FALSE && k == Value.FALSE ? curVal : (j == Value.FALSE && k == Value.TRUE ? Value.FALSE : (j == Value.TRUE && k == Value.FALSE ? Value.TRUE : (j == Value.TRUE && k == Value.TRUE ? (curVal == Value.TRUE ? Value.FALSE : (curVal == Value.FALSE ? Value.TRUE : Value.UNKNOWN)) : Value.UNKNOWN)));
            ...
            if (changed || !loc.equals(myState.propLocation)) {
                ...
                state.setValue(this.getEndLocation(3), myState.curValue, this, 6);
                state.setValue(this.getEndLocation(4), myState.curValue.not(), this, 6);
            }
        }
        ...
        public void draw(ComponentDrawContext context) {
            ...
            context.drawPin(this, 0, "J", Direction.EAST);
            context.drawClock(this, 2, Direction.EAST);
            context.drawPin(this, 1, "K", Direction.EAST);
            context.drawPin(this, 3, "Q", Direction.WEST);
            context.drawPin(this, 4);
        }
    ...
    implements Caret {
        ...
        public void mouseReleased(MouseEvent e) {
            if (this.isPressed && this.isInside(e.getX(), e.getY())) {
                ...
                this.state.setValue(this.comp.getEndLocation(3), myState.curValue, this.comp, 1);
                this.state.setValue(this.comp.getEndLocation(4), myState.curValue.not(), this.comp, 1);
            }
