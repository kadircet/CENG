...
class NLayerNet(object):
  ...
  def __init__(self, sizes, std=1e-4):
    """
    ...
    for i in range(len(sizes)-1):
        self.params.append(np.random.randn(sizes[i], sizes[i+1])*std)
        self.params[i][sizes[i]-1,:] = np.zeros(sizes[i+1])
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    for i in range(len(self.params)-1):
        Xh.append(np.maximum(0, Xh[-1].dot(self.params[i])))
    scores = Xh[-1].dot(self.params[-1])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    scores -= y
    loss = .5*np.sum(np.square(scores))
    for x in self.params:
        loss += .5*reg*np.sum(x**2)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    for i in range(len(self.params)-1,-1,-1):
        grads.append(Xh[i].T.dot(scores) + reg*self.params[i])
        scores = scores.dot(self.params[i].T)
        scores[Xh[i]==0] = 0
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      for i in range(len(grads)):
          self.params[i] -= learning_rate*grads[-i-1]
      #########################################################################
      #                             END OF YOUR CODE                          #
      #########################################################################
  ...
  def predict(self, X):
    ...
    if len(X.shape)==1:
        ...
        y_pred = np.max(self.loss(X), axis=1).reshape(X.shape[0], 1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
