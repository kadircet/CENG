...
class SeqNN(object):
  def __init__(self, word_to_idx, wordvec_dim=128, hidden_dim=128, 
    ...
    self.params['W_vocab'] = np.random.randn((10+1)*hidden_dim, vocab_size)
  ...
  def loss(self, stories, sample=False):
    ...
    for idx in xrange(stories.shape[0]):
      ...
      h1 = h[:, -1, :].reshape(1,h.shape[0]*h.shape[2]) # get only the last output
  
      X2 = stories[idx, T:T+T3].reshape(1,T3) #get the query

      h0 = np.zeros((1, Wh2.shape[0]))
      ...
      dscores1, dscores2 = dscores[:, :-Wh2.shape[0]], dscores[:, -Wh2.shape[0]:]
      dscores1 = np.vstack(np.split(dscores1, 10, axis=1))
      func = rnn_backward
      if self.cell_type=='lstm':
          func = lstm_backward
      dX_in2, dh0, _Wx, _Wh, _b = func(dscores2, cache_fwd2)
      grads['Wx2'] += _Wx
      grads['Wh2'] += _Wh
      grads['b2'] += _b
